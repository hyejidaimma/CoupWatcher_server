...
class ItemThread(threading.Thread):
    ...
    def crawl_product_price(self):
        try:
            ...
            if price_element:
                ...
                self.client_socket.send(str(crawled_price).encode())
                current_time = time.strftime("%H:%M")
                self.hourly_prices.append(crawled_price)
                return crawled_price
            else:
                return None
    ...
    def run(self):
        schedule.every().day.at(self.daily_average_time).do(self.calculate_daily_avg)
        for hour in range(24):
            time_str = f"{hour:02d}:00"
            schedule.every().day.at(time_str).do(self.crawl_product_price).tag(time_str)
